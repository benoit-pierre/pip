...
@pytest.mark.network
def test_pip_wheel_builds_when_no_binary_set(script, data, wheel_installed):
    ...
    res = script.pip(
        ...
        '-f', data.find_links, 'simple==3.0'
    )
    assert "Running setup.py bdist_wheel for simple" in str(res), str(res)


@pytest.mark.network
def test_pip_wheel_builds_editable_deps(script, data, wheel_installed):
    """
...
@pytest.mark.network
def test_pip_wheel_builds_editable(script, data, wheel_installed):
    """
...
def test_pip_wheel_with_user_set_in_config(script, data, common_wheels, wheel_installed):
    config_file = script.scratch_path / 'pip.conf'
    script.environ['PIP_CONFIG_FILE'] = str(config_file)
    config_file.write("[install]\nuser = true")
    result = script.pip(
        'wheel', '-f', common_wheels, data.src / 'withpyproject',
    )
